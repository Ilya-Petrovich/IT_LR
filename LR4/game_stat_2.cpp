...
int main () {
	double f = 0, sp = 0, s = 0, d = 360, dt = 0, dt2 = 0, fuelRemained = 0, fuelConsumed = 0, fuelConsumed2 = 0,
	fuelConsumption = 0, plus = 0, a = 0, b = 0, c = 0, sp2 = 0, fuelRemained2 = 0;
	...
	if (Answer == 'y') {
		...
		switch (Answer) {
			...
			case 'c':
				...
				break;
			case 'd':
				cout << "\nHow much fuel do you need? "; cin >> plus; f += plus;
				break;
		}
	}
	...
	sp2 = (-b * 10) / (a * 2); //Оптимальная скорость
	s = sp2 / 10;
	fuelConsumption = a * s * s + b * s + c;
