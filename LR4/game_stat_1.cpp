...
int main() {
	float s, speed, fuel, fuel_remained, fuel_capacity, fuel_consumed, fuel_consumption, distanc, distance_traveled;
	...
	if (fuel >= 0 && fuel <= fuel_capacity) {
		...
		cout << "Route info\nDistance: " << distanc << " km" << endl;
		cout << "----------------------------------------" << endl;
		cout << "You have to drive a route " << distanc << " km long.\nBut first you need to check if you have enough fuel." << endl;
		...
		if (speed >= 0) {
			...
			if (distance_traveled > distanc) {
				fuel_consumed = distanc * fuel_consumption * 0.01;
				fuel_remained = fuel - fuel_consumed;
				distance_traveled = distanc;
			}
			...
			if (distance_traveled == distanc) {
				...
				cout << "You haven't reached destination point. " << distanc - distance_traveled << " km left." << endl;
			}
		}
		else {
			cout << endl;
