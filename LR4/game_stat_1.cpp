...
int main()
{
	...
	double speedMS, fullDistance, fuel, fuelCapacity = 50.0, s = 0, distance = 360, distanceTraveled = 0;
	double fuelConsumption = 0, fuelConsumed = 0;
	...
	if (fuel >= 0 && fuel <= fuelCapacity)
	{
		...
		speedMS = (double)speed / 3.6;
                speed == 0 ? time = 0 : time = (double)distanceTraveled * 1000 / speedMS; 
