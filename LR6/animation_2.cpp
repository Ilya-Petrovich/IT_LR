#include<iostream>
#include<vector>
#include<string>
#include<unistd.h>
using namespace std;
void print(vector<string>a)
{
	for (auto c : a)
		cout << c << endl;
}
void ball()
{
	vector<string> a = { string(40,'*'),
					  "*" + string(38,' ') + "*",
		"*" + string(38,' ') + "*",
		"*" + string(38,' ') + "*",
		"*" + string(38,' ') + "*",
		"*" + string(38,' ') + "*",
		"*" + string(38,' ') + "*",
					  "*   ()()   " + string(28,' ') + "*",//0
					  "*   ()()   " + string(28,' ') + "*",//1
					  "*   ()()   " + string(28,' ') + "*",//2
					  "*   |..|   " + string(28,' ') + "*",//3
					  "*  \\|  |/  " + string(28,' ') + "*",//4
					  "*  /|UU|\\  " + string(28,' ') + "*",//5
					  "*  --\\/--  " + string(28,' ') + "*",//6
					  "* /  ||  \\ " + string(28,' ') + "*",//7
					  "*/   ||   \\" + string(28,' ') + "*",//8
					  "*    /\\    " + string(28,' ') + "*",//9
					  "*    ||    " + string(28,' ') + "*",//10
					  "*   _||_   " + string(28,' ') + "*",
	string(40,'*') };//11
	print(a);
	a[14] = "* /  ||  \\o" + string(28, ' ') + "*";
	a[15] = "*/   ||   -" + string(28, ' ') + "*";
	print(a);
	a[13] = "*  --\\/-- o" + string(28, ' ') + "*";
	a[14] = "* /  ||  \\-" + string(28, ' ') + "*";
	a[15] = "*/   ||    " + string(28, ' ') + "*";
	print(a);
	a[12] = "*  /|UU|\\ o" + string(28, ' ') + "*";
	a[13] = "*  --\\/----" + string(28, ' ') + "*";
	a[14] = "* /  ||    " + string(28, ' ') + "*";
	print(a);
	
	a[11] = "*  \\|  |/ o" + string(28, ' ') + "*";
	a[12] = "*  /|UU|\\  " + string(28, ' ') + "*";
	a[13] = "*  --\\/---/" + string(28, ' ') + "*";
	print(a);
	
	a[10] = "*   |..|  o" + string(28, ' ') + "*";
	a[11] = "*  \\|  |/  " + string(28, ' ') + "*";
	a[13] = "*  --\\/----" + string(28, ' ') + "*";
	print(a);
	
	a[9] = "*   ()()  o" + string(28, ' ') + "*";
	a[10] = "*   |..|   " + string(28, ' ') + "*";
	a[13] = "*  --\\/--- " + string(28, ' ') + "*";
	a[14] = "* /  ||   \\" + string(28, ' ') + "*";
	print(a);
	
	a[13] = "*  --\\/--  " + string(28, ' ') + "*";
	a[14] = "* /  ||  \\ " + string(28, ' ') + "*";
	a[15] = "*/   ||   \\" + string(28, ' ') + "*";
	a[9]= "*   ()()   " + string(28, ' ') + "*";
	a[8] = "*   ()()  o" + string(28, ' ') + "*";
	print(a);
	
	a[8] = "*   ()()   " + string(28, ' ') + "*";
	a[7] = "*   ()()  o" + string(28, ' ') + "*";
	print(a);
	
	a[7] = "*   ()()   " + string(28, ' ') + "*";
	a[6] = "*" + string(8, ' ') + "o" + string(29, ' ') + "*";
	print(a);
	
	a[5] = "*" + string(7, ' ') + "o" + string(30, ' ') + "*";
	a[6] = "*" + string(38, ' ') + "*";
	print(a);
	
	a[4] = "*" + string(6, ' ') + "o" + string(31, ' ') + "*";
	a[5] = "*" + string(38, ' ') + "*";
	print(a);
	
	a[3] = "*" + string(5, ' ') + "o" + string(32, ' ') + "*";
	a[4]="*"+string(38,' ')+"*";
	print(a);
	
	a[3]="*"+string(4,' ')+"o"+string(33,' ')+"*";
	print(a);
	
	a[3] = "*" + string(38, ' ') + "*";
	a[4] = "*" + string(3, ' ') + "o" + string(34, ' ') + "*";
	print(a);
	
	a[4] = "*" + string(38, ' ') + "*";
	a[5] = "*  o" + string(35, ' ') + "*";
	print(a);
	a[5]="*"+string(38,' ')+"*";
	a[6]="* o"+string(36,' ')+"*";
	print(a);
	
	a[6] = "*" + string(38, ' ') + "*";
	a[7] = "*o  ()()   " + string(28, ' ') + "*";
	print(a);
	
	a[7] = "*   ()()   " + string(28, ' ') + "*";
	a[8] = "*o  ()()   " + string(28, ' ') + "*";
	print(a);
	
	a[8] = "*   ()()   " + string(28, ' ') + "*";
	a[9] = "*o  ()()   " + string(28, ' ') + "*";
	a[13] = "* ---\\/--  " + string(28, ' ') + "*";
	a[14] = "*/   ||  \\ " + string(28, ' ') + "*";
	a[15] = "*    ||   \\" + string(28, ' ') + "*";
	print(a);
	
	a[9] = "*   ()()   " + string(28, ' ') + "*";
	a[10] = "*o  |..|   " + string(28, ' ') + "*";
	a[13] = "*----\\/--  " + string(28, ' ') + "*";
	a[14] = "*    ||  \\ " + string(28, ' ') + "*";
	print(a);
	
	a[10] = "*   |..|   " + string(28, ' ') + "*";
	a[11] = "*o \\|  |/  " + string(28, ' ') + "*";
	a[13] = "*\\---\\/--  " + string(28, ' ') + "*";
	print(a);
	
	a[11] = "*  \\|  |/  " + string(28, ' ') + "*";
	a[13]= "*----\\/--  " + string(28, ' ') + "*";
	a[12] = "*o /|UU|\\  " + string(28, ' ') + "*";
	print(a);
	
	a[12] = "*  /|UU|\\  " + string(28, ' ') + "*";
	a[13] = "*o --\\/--  " + string(28, ' ') + "*";
	a[14] = "*-/  ||  \\ " + string(28, ' ') + "*";
	print(a);
	
	a[13] = "*  --\\/--  " + string(28, ' ') + "*";
	a[14] = "*o/  ||  \\ " + string(28, ' ') + "*";
	a[15] = "*-   ||   \\" + string(28, ' ') + "*";
	print(a);
	
	a[14] = "* /  ||  \\ " + string(28, ' ') + "*";
	a[15] = "*/   ||   \\" + string(28, ' ') + "*";
	print(a);
	
}
void jump()
{
	vector<string> a = { string(40,'*'),
			"*" + string(38,' ') + "*",
			"*" + string(38,' ') + "*",
			"*" + string(38,' ') + "*",
			"*" + string(38,' ') + "*",
			"*" + string(38,' ') + "*",
			"*" + string(38,' ') + "*",
			"*   ()()   " + string(28,' ') + "*",//0
			"*   ()()   " + string(28,' ') + "*",//1
			"*   ()()   " + string(28,' ') + "*",//2
			"*   |..|   " + string(28,' ') + "*",//3
			"*  \\|  |/  " + string(28,' ') + "*",//4
			"*  /|UU|\\  " + string(28,' ') + "*",//5
			"*  --\\/--  " + string(28,' ') + "*",//6
			"* /  ||  \\ " + string(28,' ') + "*",//7
			"*/   ||   \\" + string(28,' ') + "*",//8
			"*    /\\    " + string(28,' ') + "*",//9
			"*    ||    " + string(28,' ') + "*",//10
			"*   _||_   " + string(28,' ') + "*",
			string(40,'*') };//11
	print(a);
	a[0]=string(40,'*');
	a[1]="*" + string(38,' ') + "*";
	a[2]="*" + string(38,' ') + "*";
	a[3]="*" + string(38,' ') + "*";
	a[4]="*" + string(38,' ') + "*";
	a[5]="*" + string(38,' ') + "*";
	a[6]="*" + string(38,' ') + "*";
	a[7]="*" + string(38,' ') + "*";
	a[8]="*   ()()   " + string(28,' ') + "*";
	a[9]="*   ()()   " + string(28,' ') + "*";
	a[10]="*   ()()   " + string(28,' ') + "*";
	a[11]="*   |..|   " + string(28,' ') + "*";
	a[12]="*  \\|  |/  " + string(28,' ') + "*";
	a[13]="*  /|UU|\\  " + string(28,' ') + "*";
	a[14]="*  --\\/--  " + string(28,' ') + "*";
	a[15]="* /  ||  \\ " + string(28,' ') + "*";
	a[16]="*/   ||   \\" + string(28,' ') + "*";
	a[17]="*    /\\    " + string(28,' ') + "*";
	a[18]="*   _\\/_   " + string(28,' ') + "*";
	a[19]=string(40,'*');
	print(a);
	a[0]=string(40,'*');
	a[1]="*" + string(38,' ') + "*";
	a[2]="*" + string(38,' ') + "*";
	a[3]="*" + string(38,' ') + "*";
	a[4]="*" + string(38,' ') + "*";
	a[5]="*" + string(38,' ') + "*";
	a[6]="*" + string(38,' ') + "*";
	a[7]="*   ()()   " + string(28,' ') + "*";
	a[8]="*   ()()   " + string(28,' ') + "*";
	a[9]="*   ()()   " + string(28,' ') + "*";
	a[10]="*   |..|   " + string(28,' ') + "*";
	a[11]="*  \\|  |/  " + string(28,' ') + "*";
	a[12]="*  /|UU|\\  " + string(28,' ') + "*";
	a[13]="*  --\\/--  " + string(28,' ') + "*";
	a[14]="* /  ||  \\ " + string(28,' ') + "*";
	a[15]="*/   ||   \\" + string(28,' ') + "*";
	a[16]="*    /\\    " + string(28,' ') + "*";
	a[17]="*    ||    " + string(28,' ') + "*";
	a[18]="*    ||    " + string(28,' ') + "*";
	a[19]=string(40,'*');
	print(a);
	a[0]=string(40,'*');
	a[1]="*" + string(38,' ') + "*";
	a[2]="*" + string(38,' ') + "*";
	a[3]="*" + string(38,' ') + "*";
	a[4]="*" + string(38,' ') + "*";
	a[5]="*" + string(38,' ') + "*";
	a[6]="*   ()()   " + string(28,' ') + "*";
	a[7]="*   ()()   " + string(28,' ') + "*";
	a[8]="*   ()()   " + string(28,' ') + "*";
	a[9]="*   |..|   " + string(28,' ') + "*";
	a[10]="*  \\|  |/  " + string(28,' ') + "*";
	a[11]="*  /|UU|\\  " + string(28,' ') + "*";
	a[12]="*  --\\/--  " + string(28,' ') + "*";
	a[13]="* /  ||  \\ " + string(28,' ') + "*";
	a[14]="*/   ||   \\" + string(28,' ') + "*";
	a[15]="*    /\\    " + string(28,' ') + "*";
	a[16]="*    ||    " + string(28,' ') + "*";
	a[17]="*    ||    " + string(28,' ') + "*";
	a[18]="*" + string(38,' ') + "*";
	a[19]=string(40,'*');
	print(a);
	a[0]=string(40,'*');
	a[1]="*" + string(38,' ') + "*";
	a[2]="*" + string(38,' ') + "*";
	a[3]="*" + string(38,' ') + "*";
	a[4]="*" + string(38,' ') + "*";
	a[5]="*   ()()   " + string(28,' ') + "*";
	a[6]="*   ()()   " + string(28,' ') + "*";
	a[7]="*   ()()   " + string(28,' ') + "*";
	a[8]="*   |..|   " + string(28,' ') + "*";
	a[9]="*  \\|  |/  " + string(28,' ') + "*";
	a[10]="*  /|UU|\\  " + string(28,' ') + "*";
	a[11]="*  --\\/--  " + string(28,' ') + "*";
	a[12]="* /  ||  \\ " + string(28,' ') + "*";
	a[13]="*/   ||   \\" + string(28,' ') + "*";
	a[14]="*    /\\    " + string(28,' ') + "*";
	a[15]="*    ||    " + string(28,' ') + "*";
	a[16]="*    ||    " + string(28,' ') + "*";
	a[17]="*" + string(38,' ') + "*";
	a[18]="*" + string(38,' ') + "*";
	a[19]=string(40,'*');
	print(a);
	a[0]=string(40,'*');
	a[1]="*" + string(38,' ') + "*";
	a[2]="*" + string(38,' ') + "*";
	a[3]="*" + string(38,' ') + "*";
	a[4]="*   ()()   " + string(28,' ') + "*";
	a[5]="*   ()()   " + string(28,' ') + "*";
	a[6]="*   ()()   " + string(28,' ') + "*";
	a[7]="*   |..|   " + string(28,' ') + "*";
	a[8]="*  \\|  |/  " + string(28,' ') + "*";
	a[9]="*  /|UU|\\  " + string(28,' ') + "*";
	a[10]="*  --\\/--  " + string(28,' ') + "*";
	a[11]="* /  ||  \\ " + string(28,' ') + "*";
	a[12]="*/   ||   \\" + string(28,' ') + "*";
	a[13]="*    /\\    " + string(28,' ') + "*";
	a[14]="*    ||    " + string(28,' ') + "*";
	a[15]="*    ||    " + string(28,' ') + "*";
	a[16]="*" + string(38,' ') + "*";
	a[17]="*" + string(38,' ') + "*";
	a[18]="*" + string(38,' ') + "*";
	a[19]=string(40,'*');
	print(a);
	a[0]=string(40,'*');
	a[1]="*" + string(38,' ') + "*";
	a[2]="*" + string(38,' ') + "*";
	a[3]="*   ()()   " + string(28,' ') + "*";
	a[4]="*   ()()   " + string(28,' ') + "*";
	a[5]="*   ()()   " + string(28,' ') + "*";
	a[6]="*   |..|   " + string(28,' ') + "*";
	a[7]="*  \\|  |/  " + string(28,' ') + "*";
	a[8]="*  /|UU|\\  " + string(28,' ') + "*";
	a[9]="*  --\\/--  " + string(28,' ') + "*";
	a[10]="* /  ||  \\ " + string(28,' ') + "*";
	a[11]="*/   ||   \\" + string(28,' ') + "*";
	a[12]="*    /\\    " + string(28,' ') + "*";
	a[13]="*    ||    " + string(28,' ') + "*";
	a[14]="*    ||    " + string(28,' ') + "*";
	a[15]="*" + string(38,' ') + "*";
	a[16]="*" + string(38,' ') + "*";
	a[17]="*" + string(38,' ') + "*";
	a[18]="*" + string(38,' ') + "*";
	a[19]=string(40,'*');
	print(a);
	a[0]=string(40,'*');
	a[1]="*" + string(38,' ') + "*";
	a[2]="*" + string(38,' ') + "*";
	a[3]="*" + string(38,' ') + "*";
	a[4]="*   ()()   " + string(28,' ') + "*";
	a[5]="*   ()()   " + string(28,' ') + "*";
	a[6]="*   ()()   " + string(28,' ') + "*";
	a[7]="*   |..|   " + string(28,' ') + "*";
	a[8]="*  \\|  |/  " + string(28,' ') + "*";
	a[9]="*  /|UU|\\  " + string(28,' ') + "*";
	a[10]="*  --\\/--  " + string(28,' ') + "*";
	a[11]="* /  ||  \\ " + string(28,' ') + "*";
	a[12]="*/   ||   \\" + string(28,' ') + "*";
	a[13]="*    /\\    " + string(28,' ') + "*";
	a[14]="*    ||    " + string(28,' ') + "*";
	a[15]="*    ||    " + string(28,' ') + "*";
	a[16]="*" + string(38,' ') + "*";
	a[17]="*" + string(38,' ') + "*";
	a[18]="*" + string(38,' ') + "*";
	a[19]=string(40,'*');
	print(a);
	a[0]=string(40,'*');
	a[1]="*" + string(38,' ') + "*";
	a[2]="*" + string(38,' ') + "*";
	a[3]="*" + string(38,' ') + "*";
	a[4]="*" + string(38,' ') + "*";
	a[5]="*   ()()   " + string(28,' ') + "*";
	a[6]="*   ()()   " + string(28,' ') + "*";
	a[7]="*   ()()   " + string(28,' ') + "*";
	a[8]="*   |..|   " + string(28,' ') + "*";
	a[9]="*  \\|  |/  " + string(28,' ') + "*";
	a[10]="*  /|UU|\\  " + string(28,' ') + "*";
	a[11]="*  --\\/--  " + string(28,' ') + "*";
	a[12]="* /  ||  \\ " + string(28,' ') + "*";
	a[13]="*/   ||   \\" + string(28,' ') + "*";
	a[14]="*    /\\    " + string(28,' ') + "*";
	a[15]="*    ||    " + string(28,' ') + "*";
	a[16]="*    ||    " + string(28,' ') + "*";
	a[17]="*" + string(38,' ') + "*";
	a[18]="*" + string(38,' ') + "*";
	a[19]=string(40,'*');
	print(a);
	a[0]=string(40,'*');
	a[1]="*" + string(38,' ') + "*";
	a[2]="*" + string(38,' ') + "*";
	a[3]="*" + string(38,' ') + "*";
	a[4]="*" + string(38,' ') + "*";
	a[5]="*" + string(38,' ') + "*";
	a[6]="*   ()()   " + string(28,' ') + "*";
	a[7]="*   ()()   " + string(28,' ') + "*";
	a[8]="*   ()()   " + string(28,' ') + "*";
	a[9]="*   |..|   " + string(28,' ') + "*";
	a[10]="*  \\|  |/  " + string(28,' ') + "*";
	a[11]="*  /|UU|\\  " + string(28,' ') + "*";
	a[12]="*  --\\/--  " + string(28,' ') + "*";
	a[13]="* /  ||  \\ " + string(28,' ') + "*";
	a[14]="*/   ||   \\" + string(28,' ') + "*";
	a[15]="*    /\\    " + string(28,' ') + "*";
	a[16]="*    ||    " + string(28,' ') + "*";
	a[17]="*    ||    " + string(28,' ') + "*";
	a[18]="*" + string(38,' ') + "*";
	a[19]=string(40,'*');
	print(a);
	a[0]=string(40,'*');
	a[1]="*" + string(38,' ') + "*";
	a[2]="*" + string(38,' ') + "*";
	a[3]="*" + string(38,' ') + "*";
	a[4]="*" + string(38,' ') + "*";
	a[5]="*" + string(38,' ') + "*";
	a[6]="*" + string(38,' ') + "*";
	a[7]="*   ()()   " + string(28,' ') + "*";
	a[8]="*   ()()   " + string(28,' ') + "*";
	a[9]="*   ()()   " + string(28,' ') + "*";
	a[10]="*   |..|   " + string(28,' ') + "*";
	a[11]="*  \\|  |/  " + string(28,' ') + "*";
	a[12]="*  /|UU|\\  " + string(28,' ') + "*";
	a[13]="*  --\\/--  " + string(28,' ') + "*";
	a[14]="* /  ||  \\ " + string(28,' ') + "*";
	a[15]="*/   ||   \\" + string(28,' ') + "*";
	a[16]="*    /\\    " + string(28,' ') + "*";
	a[17]="*    ||    " + string(28,' ') + "*";
	a[18]="*    ||    " + string(28,' ') + "*";
	a[19]=string(40,'*');
	print(a);
	a[0]=string(40,'*');
	a[1]="*" + string(38,' ') + "*";
	a[2]="*" + string(38,' ') + "*";
	a[3]="*" + string(38,' ') + "*";
	a[4]="*" + string(38,' ') + "*";
	a[5]="*" + string(38,' ') + "*";
	a[6]="*" + string(38,' ') + "*";
	a[7]="*" + string(38,' ') + "*";
	a[8]="*   ()()   " + string(28,' ') + "*";
	a[9]="*   ()()   " + string(28,' ') + "*";
	a[10]="*   ()()   " + string(28,' ') + "*";
	a[11]="*   |..|   " + string(28,' ') + "*";
	a[12]="*  \\|  |/  " + string(28,' ') + "*";
	a[13]="*  /|UU|\\  " + string(28,' ') + "*";
	a[14]="*  --\\/--  " + string(28,' ') + "*";
	a[15]="* /  ||  \\ " + string(28,' ') + "*";
	a[16]="*/   ||   \\" + string(28,' ') + "*";
	a[17]="*    /\\    " + string(28,' ') + "*";
	a[18]="*   _\\/_   " + string(28,' ') + "*";
	a[19]=string(40,'*');
	print(a);
	a[0]=string(40,'*');
	a[1]="*" + string(38,' ') + "*";
	a[2]="*" + string(38,' ') + "*";
	a[3]="*" + string(38,' ') + "*";
	a[4]="*" + string(38,' ') + "*";
	a[5]="*" + string(38,' ') + "*";
	a[6]="*" + string(38,' ') + "*";
	a[7]="*   ()()   " + string(28,' ') + "*";
	a[8]="*   ()()   " + string(28,' ') + "*";
	a[9]="*   ()()   " + string(28,' ') + "*";
	a[10]="*   |..|   " + string(28,' ') + "*";
	a[11]="*  \\|  |/  " + string(28,' ') + "*";
	a[12]="*  /|UU|\\  " + string(28,' ') + "*";
	a[13]="*  --\\/--  " + string(28,' ') + "*";
	a[14]="* /  ||  \\ " + string(28,' ') + "*";
	a[15]="*/   ||   \\" + string(28,' ') + "*";
	a[16]="*    /\\    " + string(28,' ') + "*";
	a[17]="*    ||    " + string(28,' ') + "*";
	a[18]="*   _||_   " + string(28,' ') + "*";
	a[19]=string(40,'*');
	print(a);
}

void run()
{
	vector<string> a = { string(40,'*'),
					  "*" + string(38,' ') + "*",
		"*" + string(38,' ') + "*",
		"*" + string(38,' ') + "*",
		"*" + string(38,' ') + "*",
		"*" + string(38,' ') + "*",
		"*" + string(38,' ') + "*",
					  "*   ()()   " + string(28,' ') + "*",//0
					  "*   ()()   " + string(28,' ') + "*",//1
					  "*   ()()   " + string(28,' ') + "*",//2
					  "*   |..|   " + string(28,' ') + "*",//3
					  "*  \\|  |/  " + string(28,' ') + "*",//4
					  "*  /|UU|\\  " + string(28,' ') + "*",//5
					  "*  --\\/--  " + string(28,' ') + "*",//6
					  "* /  ||  \\ " + string(28,' ') + "*",//7
					  "*/   ||   \\" + string(28,' ') + "*",//8
					  "*    /\\    " + string(28,' ') + "*",//9
					  "*    ||    " + string(28,' ') + "*",//10
					  "*   _||_   " + string(28,' ') + "*",
	string(40,'*') };//11
	print(a);
	
	int k = 0;
	while (k < 28)
	{
		a[7] = "*" + string(k, ' ') + "   ()()   " + string(28 - k, ' ') + "*";
		a[8] = "*" + string(k, ' ') + "   ()()   " + string(28 - k, ' ') + "*";
		a[9] = "*" + string(k, ' ') + "   ()()   " + string(28 - k, ' ') + "*";
		a[10] = "*" + string(k, ' ') + "   |..|   " + string(28 - k, ' ') + "*";
		a[11] = "*" + string(k, ' ') + "  \\|  |/  " + string(28 - k, ' ') + "*";
		a[12] = "*" + string(k, ' ') + "  /|UU|\\  " + string(28 - k, ' ') + "*";
		a[13] = "*" + string(k, ' ') + "  --\\/--  " + string(28 - k, ' ') + "*";
		a[14] = "*" + string(k, ' ') + " /  ||  \\ " + string(28 - k, ' ') + "*";
		a[15] = "*" + string(k, ' ') + "/   ||   \\" + string(28 - k, ' ') + "*";
		a[16] = "*" + string(k, ' ') + "    /\\    " + string(28 - k, ' ') + "*";
		a[17] = "*" + string(k, ' ') + "    |\\    " + string(28 - k, ' ') + "*";
		a[18] = "*" + string(k, ' ') + "   _| \\_  " + string(28 - k, ' ') + "*";
		print(a);
		
		k += 1;
		a[7] = "*" + string(k, ' ') + "   ()()   " + string(28 - k, ' ') + "*";
		a[8] = "*" + string(k, ' ') + "   ()()   " + string(28 - k, ' ') + "*";
		a[9] = "*" + string(k, ' ') + "   ()()   " + string(28 - k, ' ') + "*";
		a[10] = "*" + string(k, ' ') + "   |..|   " + string(28 - k, ' ') + "*";
		a[11] = "*" + string(k, ' ') + "  \\|  |/  " + string(28 - k, ' ') + "*";
		a[12] = "*" + string(k, ' ') + "  /|UU|\\  " + string(28 - k, ' ') + "*";
		a[13] = "*" + string(k, ' ') + "  --\\/--  " + string(28 - k, ' ') + "*";
		a[14] = "*" + string(k, ' ') + " /  ||  \\ " + string(28 - k, ' ') + "*";
		a[15] = "*" + string(k, ' ') + "/   ||   \\" + string(28 - k, ' ') + "*";
		a[16] = "*" + string(k, ' ') + "    /\\    " + string(28 - k, ' ') + "*";
		a[17] = "*" + string(k, ' ') + "    /|    " + string(28 - k, ' ') + "*";
		a[18] = "*" + string(k, ' ') + "  _/ |_   " + string(28 - k, ' ') + "*";
		print(a);
		
		a[7] = "*" + string(k, ' ') + "   ()()   " + string(28 - k, ' ') + "*";
		a[8] = "*" + string(k, ' ') + "   ()()   " + string(28 - k, ' ') + "*";
		a[9] = "*" + string(k, ' ') + "   ()()   " + string(28 - k, ' ') + "*";
		a[10] = "*" + string(k, ' ') + "   |..|   " + string(28 - k, ' ') + "*";
		a[11] = "*" + string(k, ' ') + "  \\|  |/  " + string(28 - k, ' ') + "*";
		a[12] = "*" + string(k, ' ') + "  /|UU|\\  " + string(28 - k, ' ') + "*";
		a[13] = "*" + string(k, ' ') + "  --\\/--  " + string(28 - k, ' ') + "*";
		a[14] = "*" + string(k, ' ') + " /  ||  \\ " + string(28 - k, ' ') + "*";
		a[15] = "*" + string(k, ' ') + "/   ||   \\" + string(28 - k, ' ') + "*";
		a[16] = "*" + string(k, ' ') + "    /\\    " + string(28 - k, ' ') + "*";
		a[17] = "*" + string(k, ' ') + "    ||    " + string(28 - k, ' ') + "*";
		a[18] = "*" + string(k, ' ') + "   _||_   " + string(28 - k, ' ') + "*";
		print(a);
		
	}
	while (k > 1)
	{
		a[7] = "*" + string(k, ' ') + "   ()()   " + string(28 - k, ' ') + "*";
		a[8] = "*" + string(k, ' ') + "   ()()   " + string(28 - k, ' ') + "*";
		a[9] = "*" + string(k, ' ') + "   ()()   " + string(28 - k, ' ') + "*";
		a[10] = "*" + string(k, ' ') + "   |..|   " + string(28 - k, ' ') + "*";
		a[11] = "*" + string(k, ' ') + "  \\|  |/  " + string(28 - k, ' ') + "*";
		a[12] = "*" + string(k, ' ') + "  /|UU|\\  " + string(28 - k, ' ') + "*";
		a[13] = "*" + string(k, ' ') + "  --\\/--  " + string(28 - k, ' ') + "*";
		a[14] = "*" + string(k, ' ') + " /  ||  \\ " + string(28 - k, ' ') + "*";
		a[15] = "*" + string(k, ' ') + "/   ||   \\" + string(28 - k, ' ') + "*";
		a[16] = "*" + string(k, ' ') + "    /\\    " + string(28 - k, ' ') + "*";
		a[17] = "*" + string(k, ' ') + "    |\\    " + string(28 - k, ' ') + "*";
		a[18] = "*" + string(k, ' ') + "   _| \\_  " + string(28 - k, ' ') + "*";
		print(a);
		
		k -= 1;
		a[7] = "*" + string(k, ' ') + "   ()()   " + string(28 - k, ' ') + "*";
		a[8] = "*" + string(k, ' ') + "   ()()   " + string(28 - k, ' ') + "*";
		a[9] = "*" + string(k, ' ') + "   ()()   " + string(28 - k, ' ') + "*";
		a[10] = "*" + string(k, ' ') + "   |..|   " + string(28 - k, ' ') + "*";
		a[11] = "*" + string(k, ' ') + "  \\|  |/  " + string(28 - k, ' ') + "*";
		a[12] = "*" + string(k, ' ') + "  /|UU|\\  " + string(28 - k, ' ') + "*";
		a[13] = "*" + string(k, ' ') + "  --\\/--  " + string(28 - k, ' ') + "*";
		a[14] = "*" + string(k, ' ') + " /  ||  \\ " + string(28 - k, ' ') + "*";
		a[15] = "*" + string(k, ' ') + "/   ||   \\" + string(28 - k, ' ') + "*";
		a[16] = "*" + string(k, ' ') + "    /\\    " + string(28 - k, ' ') + "*";
		a[17] = "*" + string(k, ' ') + "    /|    " + string(28 - k, ' ') + "*";
		a[18] = "*" + string(k, ' ') + "  _/ |_   " + string(28 - k, ' ') + "*";
		print(a);
		
		a[7] = "*" + string(k, ' ') + "   ()()   " + string(28 - k, ' ') + "*";
		a[8] = "*" + string(k, ' ') + "   ()()   " + string(28 - k, ' ') + "*";
		a[9] = "*" + string(k, ' ') + "   ()()   " + string(28 - k, ' ') + "*";
		a[10] = "*" + string(k, ' ') + "   |..|   " + string(28 - k, ' ') + "*";
		a[11] = "*" + string(k, ' ') + "  \\|  |/  " + string(28 - k, ' ') + "*";
		a[12] = "*" + string(k, ' ') + "  /|UU|\\  " + string(28 - k, ' ') + "*";
		a[13] = "*" + string(k, ' ') + "  --\\/--  " + string(28 - k, ' ') + "*";
		a[14] = "*" + string(k, ' ') + " /  ||  \\ " + string(28 - k, ' ') + "*";
		a[15] = "*" + string(k, ' ') + "/   ||   \\" + string(28 - k, ' ') + "*";
		a[16] = "*" + string(k, ' ') + "    /\\    " + string(28 - k, ' ') + "*";
		a[17] = "*" + string(k, ' ') + "    ||    " + string(28 - k, ' ') + "*";
		a[18] = "*" + string(k, ' ') + "   _||_   " + string(28 - k, ' ') + "*";
		print(a);
		
	}
	a[7] = "*" + string(1, ' ') + "   ()()   " + string(27, ' ') + "*";
	a[8] = "*" + string(1, ' ') + "   ()()   " + string(27, ' ') + "*";
	a[9] = "*" + string(1, ' ') + "   ()()   " + string(27, ' ') + "*";
	a[10] = "*" + string(1, ' ') + "   |..|   " + string(27, ' ') + "*";
	a[11] = "*" + string(1, ' ') + "  \\|  |/  " + string(27, ' ') + "*";
	a[12] = "*" + string(1, ' ') + "  /|UU|\\  " + string(27, ' ') + "*";
	a[13] = "*" + string(1, ' ') + "  --\\/--  " + string(27, ' ') + "*";
	a[14] = "*" + string(1, ' ') + " /  ||  \\ " + string(27, ' ') + "*";
	a[15] = "*" + string(1, ' ') + "/   ||   \\" + string(27, ' ') + "*";
	a[16] = "*" + string(1, ' ') + "    /\\    " + string(27, ' ') + "*";
	a[17] = "*" + string(1, ' ') + "    |\\    " + string(27, ' ') + "*";
	a[18] = "*" + string(1, ' ') + "   _| \\_  " + string(27, ' ') + "*";
	print(a);
	a[7] = "*   ()()   " + string(28, ' ') + "*";
	a[8] = "*   ()()   " + string(28, ' ') + "*";
	a[9] = "*   ()()   " + string(28, ' ') + "*";
	a[10] = "*   |..|   " + string(28, ' ') + "*";
	a[11] = "*  \\|  |/  " + string(28, ' ') + "*";
	a[12] = "*  /|UU|\\  " + string(28, ' ') + "*";
	a[13] = "*  --\\/--  " + string(28, ' ') + "*";
	a[14] = "* /  ||  \\ " + string(28, ' ') + "*";
	a[15] = "*/   ||   \\" + string(28, ' ') + "*";
	a[16] = "*    /\\    " + string(28, ' ') + "*";
	a[17] = "*    ||    " + string(28, ' ') + "*";
	a[18] = "*   _||_   " + string(28, ' ') + "*";
	print(a);
}
void dance()
{
	vector<string> a = { string(40,'*'),
					  "*" + string(38,' ') + "*",
		"*" + string(38,' ') + "*",
		"*" + string(38,' ') + "*",
		"*" + string(38,' ') + "*",
		"*" + string(38,' ') + "*",
		"*" + string(38,' ') + "*",
					  "*   ()()   " + string(28,' ') + "*",//0
					  "*   ()()   " + string(28,' ') + "*",//1
					  "*   ()()   " + string(28,' ') + "*",//2
					  "*   |..|   " + string(28,' ') + "*",//3
					  "*  \\|  |/  " + string(28,' ') + "*",//4
					  "*  /|UU|\\  " + string(28,' ') + "*",//5
					  "*  --\\/--  " + string(28,' ') + "*",//6
					  "* /  ||  \\ " + string(28,' ') + "*",//7
					  "*/   ||   \\" + string(28,' ') + "*",//8
					  "*    /\\    " + string(28,' ') + "*",//9
					  "*    ||    " + string(28,' ') + "*",//10
					  "*   _||_   " + string(28,' ') + "*",
	string(40,'*') };//11
	int i = 0;
	print(a);
	
	while (i < 2)
	{
		
		a[13] = "* ---\\/--- " + string(28, ' ') + "*";
		a[14] = "*/   ||   \\" + string(28, ' ') + "*";
		a[15] = "*    ||    " + string(28, ' ') + "*";
		print(a);
		
		a[13] = "*  --\\/----" + string(28, ' ') + "*";
		a[14] = "* /  ||    " + string(28, ' ') + "*";
		a[15] = "*/   ||    " + string(28, ' ') + "*";
		print(a);
		
		a[12] = "*  /|UU|\\ /" + string(28, ' ') + "*";
		a[13] = "* ---\\/--- " + string(28, ' ') + "*";
		a[14] = "*/   ||    " + string(28, ' ') + "*";
		a[15] = "*    ||    " + string(28, ' ') + "*";
		print(a);
		
		a[11] = "*  \\|  |/ /" + string(28, ' ') + "*";
		a[12] = "*  /|UU|\\/ " + string(28, ' ') + "*";
		a[13] = "*  --\\/--  " + string(28, ' ') + "*";
		a[14] = "* /  ||    " + string(28, ' ') + "*";
		a[15] = "*/   ||    " + string(28, ' ') + "*";
		print(a);
		
		a[11] = "*  \\|  |/ *" + string(28, ' ') + "*";
		a[13] = "* ---\\/--  " + string(28, ' ') + "*";
		a[14] = "*/   ||    " + string(28, ' ') + "*";
		a[15] = "*    ||    " + string(28, ' ') + "*";
		print(a);
		
		a[11] = "*  \\|  |/ /" + string(28, ' ') + "*";
		a[13] = "*----\\/--  " + string(28, ' ') + "*";
		a[14] = "*    ||    " + string(28, ' ') + "*";
		a[15] = "*    ||    " + string(28, ' ') + "*";
		print(a);
		
		a[11] = "*  \\|  |/ *" + string(28, ' ') + "*";
		a[12] = "*\\ /|UU|\\/ " + string(28, ' ') + "*";
		a[13] = "* ---\\/--  " + string(28, ' ') + "*";
		print(a);
		
		a[11] = "*\\ \\|  |/ /" + string(28, ' ') + "*";
		a[12] = "* \\/|UU|\\/ " + string(28, ' ') + "*";
		a[13] = "*  --\\/--  " + string(28, ' ') + "*";
		print(a);
		
		a[11] = "** \\|  |/ *" + string(28, ' ') + "*";
		print(a);
		
		a[11] = "*  \\|  |/  " + string(28, ' ') + "*";
		a[12] = "** /|UU|\\ *" + string(28, ' ') + "*";
		a[13] = "* ---\\/--- " + string(28, ' ') + "*";
		print(a);
		
		a[12] = "*  /|UU|\\  " + string(28, ' ') + "*";
		a[13] = "**---\\/---*" + string(28, ' ') + "*";
		print(a);
		
		a[13] = "* ---\\/--- " + string(28, ' ') + "*";
		a[14] = "**   ||   *" + string(28, ' ') + "*";
		print(a);
		
		a[13] = "*  --\\/--  " + string(28, ' ') + "*";
		a[14] = "* /  ||  \\ " + string(28, ' ') + "*";
		a[15] = "**   ||   *" + string(28, ' ') + "*";
		print(a);
		
		a[15] = "*/   ||   \\" + string(28, ' ') + "*";
		print(a);
		
		i++;
	}
}

int main()
{
L1:int t;
	cout << "Choose animation:" << endl;
	cout << "Dance: \t\t\t1" << endl;
	cout << "Run: \t\t\t2" << endl;
	cout << "Sit down and jump: \t3" << endl;
	cout << "Throw a ball: \t\t4" << endl << endl;
	cout << "Enter 5 for exit." << endl;
	
	cin >> t;
	switch (t)
	{
	case 1: {dance();goto L1; }
	case 3 {run();	goto L1; }
	case 2: {jump();goto L1; }
	case 4: {ball(); goto L1; }
	case 5: {return 0; }
	}
}